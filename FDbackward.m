function Uh=FDbackward(alfa,h,mu,n,u0,uL)

    diag = (2*mu/(h^2)+alfa/h)*ones(1,n-2);
    offdiaginf = (-mu/(h^2)-alfa/h)*ones(1,n-2); % spdiags ignores last value.
    offdiagsup = (-mu/(h^2))*ones(1,n-2); % spdiags ignores first value.
