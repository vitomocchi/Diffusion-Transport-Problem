function Uh=FDcentr(alfa,h,mu,n,u0,uL)

    diag = 2*mu/(h^2)*ones(1,n-2);
    offdiaginf = (-mu/(h^2)-alfa/(2*h))*ones(1,n-2); % spdiags ignora l'ultimo valore.
    offdiagsup = (-mu/(h^2)+alfa/(2*h))*ones(1,n-2); % spdiags ignora il primo valore.
